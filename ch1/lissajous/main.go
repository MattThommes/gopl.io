...
var palette = []color.Color{blackColor, greenColor, yellowColor, redColor, blueColor}
...
func lissajous(out io.Writer) {
	...
	for i := 0; i < nframes; i++ {
		...
		for t := 0.0; t < cycles*2*math.Pi; t += res {
			...
			// Pick a random integer from the palette sequence (0-4)
			randColorIndex := rand.Intn(4)
			img.SetColorIndex(size+int(x*size+0.5), size+int(y*size+0.5),
				uint8(randColorIndex))
		}
		phase += 0.1
